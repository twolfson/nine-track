...
NineTrack.prototype = _.defaults({
    ...
    async.waterfall([
      ...
      function createRemoteReq (connInfo, cb) {
        ...
        if (that.seriesKey) {
          ...
          console.log(that.pastRequestKeys);
          if (that.pastRequestKeys.length === 0) {
...
function middlewareCreator(options) {
  ...
  keys.forEach(function bindNineTrackMethod (key) {
    // TODO: Consider proper inheritance to avoid hacks like skipping based on `key`
    if (key !== 'emit' && key !== 'on') {
      nineTrackMiddleware[key] = function executeNineTrackMethod () {
        nineTrack[key].apply(nineTrack, arguments);
      };
    } else {
      nineTrackMiddleware[key] = nineTrack[key];
    }
  });

  // Forward any errors
  nineTrack.on('error', function forwardError (err) {
    nineTrackMiddleware.emit('error', err);
  });
