...
NineTrack.prototype = {
  ...
  getConnectionKey: function (conn, pastRequestKeys) {
    ...
    if (pastRequestKeys) {
      json += this.seriesKey + '=' + pastRequestKeys.join(';');
    }
  ...
  saveConnection: function (key, _info, pastRequestKeys, cb) {
    ...
    if (this.seriesKey) {
      info.pastRequestKeys = pastRequestKeys;
    }
  ...
  forwardRequest: function (localReq, callback) {
    ...
    var pastRequestKeys, pastRequestIndex;
    if (this.pastRequestKeys) {
      pastRequestKeys = this.pastRequestKeys.slice();
      console.log('a', pastRequestKeys);
      pastRequestIndex = this.pastRequestKeys.length;
      this.pastRequestKeys[pastRequestIndex] = 'Parallel request. Unable to determine hash.';
    }
    ...
    async.waterfall([
      ...
      function findSavedConnection (cb) {
        requestKey = that.getConnectionKey(localReqMsg, pastRequestKeys);
      ...
      function createRemoteReq (connInfo, cb) {
        ...
        if (that.seriesKey) {
          ...
          // Otherwise, if this is not a new recording, consider our series corrupted
            ...
            return that.removeFixtures(pastRequestKeys, function handleRemoval (err) {
              // Prepare our message
      ...
      function saveIncomingRemote (cb) {
        ...
        that.saveConnection(requestKey, connInfo, pastRequestKeys, cb);
      }
