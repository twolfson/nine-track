...
NineTrack.prototype = {
  ...
  saveConnection: function (key, _info, cb) {
    ...
    // if (info.pas) {
    //   info.pastRequestKeys = info.request.pastRequestKeys;
    // }
    ...
    async.waterfall([
      ...
      function createRemoteReq (connInfo, cb) {
        ...
        if (that.seriesKey) {
          ...
          if (localReqMsg.pastRequestKeys.length < 1) {
          ...
          // Otherwise, if this is not a new recording, consider our series corrupted
            ...
            return that.removeFixtures(localReqMsg.pastRequestKeys, function handleRemoval (err) {
              // Prepare our message
