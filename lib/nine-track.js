...
NineTrack.prototype = {
      ...
      function createRemoteReq (connInfo, cb) {
        ...
        if (that.seriesKey) {
          ...
          // Otherwise, if this is not a new recording, consider our series corrupted
            ...
            return that.removeFixtures(that.pastRequestKeys, function handleRemoval (err) {
              // Prepare our message
  ...
  removeFixtures: function (keys, cb) {
    // TODO: Add support for `delete` inside of `fs-memory-store`
    process.nextTick(cb);
  }
};
